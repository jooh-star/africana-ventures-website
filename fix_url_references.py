...
# Function to update URL references in a file
def update_file(file_path):
    ...
    for route in admin_routes:
        pattern = r"url_for\(['\"]({})['\"]\)".format(route)
        original_matches += len(re.findall(pattern, content))
    
    # Replace url_for('route_name') with url_for('admin.route_name')
    updated_content = content
    replacements = 0
    for route in admin_routes:
        pattern = r"url_for\(['\"]({})['\"]\)".format(route)
        replacement = r"url_for('admin.\1')"
...
def process_templates():
    ...
    print(f"\nSummary:\n{updated_files} of {total_files} files updated\n{total_replacements} total URL references fixed")

if __name__ == '__main__':
    print("Starting to fix URL references in admin templates...")
    process_templates()
    print("Done!")